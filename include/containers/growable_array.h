...
namespace containers {
    ...
    class growable_array: detail::compressed_tuple<BlockAllocator, BlockMapAllocator> {
        ...
        block_map* allocate_block_map(std::size_t n) {
            if (std::numeric_limits<std::size_t>::max() / sizeof(block_type*) < n)
                return nullptr;
            if (std::numeric_limits<std::size_t>::max() - sizeof(block_map) < sizeof(block_type*) * n)
                return nullptr;
            return (block_map*)byte_allocator().allocate(sizeof(block_map) + sizeof(block_type*) * n);
        }
        void deallocate_block_map(block_map* ptr) {
            byte_allocator().deallocate((uint8_t*)ptr, sizeof(block_map) + sizeof(block_type*) * ptr->capacity);
        }

        T& read(std::size_t size, std::size_t n) {
            assert(n < size);
    ...
    public:
        ...
        T& read(reader_state& state, std::size_t n) {
            if (n >= state.size)
        ...
        std::size_t empty() const { return size_.load(std::memory_order_acquire) == 0; }

        template< typename... Args > std::size_t emplace_back(Args&&... args) {
            std::size_t size = size_.load(std::memory_order_relaxed);
            std::size_t index = size >> (detail::log2(BlockSize) - 1);
            std::size_t offset = size & (BlockSize - 1);
            ...
            if (map_size_) {
                ...
                insert:
                    ...
                    return size + 1;
                } else if (map_size_ < map->capacity) {
                    ...
                    ++map_size_;
                    new_map->capacity = capacity * BlocksGrowFactor;
                    deallocate_block_map(map);
                ...
                new_map->capacity = BlocksGrowFactor;
        ...
        std::size_t push_back(const T& value) { return emplace_back(value); }
        std::size_t push_back(T&& value) { return emplace_back(std::move(value)); }
    };
}
