...
/*
template< typename Allocator > static void growable_array_append(benchmark::State& state) {
    for (auto _ : state) {
        ResourceMark< Allocator > rm;
        containers::growable_array<int, Allocator > c;
...
// BENCHMARK_TEMPLATE(growable_array_append, std::allocator<int>)->Range(1, N)->UseRealTime();
// BENCHMARK_TEMPLATE(growable_array_append, containers::thread_arena_allocator<int>)->Range(1, N)->UseRealTime();
